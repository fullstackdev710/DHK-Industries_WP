...
class Helpers {
	...
	public static function download_import_files( $import_file_info ) {
		 ...
		 * Customizer
		 * 
		 * Upload customizer options only
		 * if the user didn't upload logo.
		 */
		if ( ! empty( $import_file_info['import_customizer_file_url'] ) && empty( get_theme_mod( 'custom_logo' ) ) ) {
		 ...
		 * Install Plugins
		 */
		if ( ! empty( $import_file_info['neuron_required_plugins'] ) ) {
			$downloaded_files['neuron_required_plugins'] = $import_file_info['neuron_required_plugins'];
		}

		/**
		 * Import Pods (JSON)
		 */
		if ( ! empty( $import_file_info['import_pods_fields'] ) ) {
			$downloaded_files['import_pods_fields'] = $import_file_info['import_pods_fields'];
		}
	...
	public static function write_to_file( $content, $file_path ) {
		...
		if ( ! $wp_filesystem->put_contents( $file_path, $content ) ) {
			return new \WP_Error(
				...
				sprintf(
					__( 'An error occurred while writing file to your server! Tried to write a file to: %s%s.', 'neuron-builder' ),
					'<br>',
					$file_path
				)
			);
	...
	public static function data_from_file( $file_path ) {
		...
		if ( ! $data ) {
			return new \WP_Error(
				...
				sprintf(
					__( 'An error occurred while reading a file from your server! Tried reading file from path: %s%s.', 'neuron-builder' ),
					'<br>',
					$file_path
				)
			);
	...
	private static function check_wp_filesystem_credentials() {
		...
		if ( ! ( 'direct' === get_filesystem_method() ) ) {
			return new \WP_Error(
				...
				sprintf(
					__( 'This WordPress page does not have %sdirect%s write file access. This plugin needs it in order to save the demo import xml file to the upload directory of your site. You can change this setting with these instructions: %s.', 'neuron-builder' ),
					'<strong>',
					'</strong>',
		...
		$plugin_page_setup = apply_filters( 'pt-ocdi/plugin_page_setup', array(
				...
				'page_title'  => esc_html__( 'One Click Demo Import' , 'neuron-builder' ),
				'menu_title'  => esc_html__( 'Import Demo Data' , 'neuron-builder' ),
				'capability'  => 'import',
		...
		if ( false === ( $creds = request_filesystem_credentials( $demo_import_page_url, '', false, false, null ) ) ) {
			return new \WP_error(
				...
				__( 'An error occurred while retrieving reading/writing permissions to your server (could not retrieve WP filesystem credentials)!', 'neuron-builder' )
			);
		}
		...
		if ( ! WP_Filesystem( $creds ) ) {
			return new \WP_Error(
				...
				__( 'Your WordPress login credentials don\'t allow to use WP_Filesystem!', 'neuron-builder' )
			);
		}
	...
	public static function register_file_as_media_attachment( $log_path ) {
		...
		$attachment = array(
			...
			'post_title'     => apply_filters( 'pt-ocdi/attachment_prefix', esc_html__( 'One Click Demo Import - ', 'neuron-builder' ) ) . preg_replace( '/\.[^.]+$/', '', basename( $log_path ) ),
			'post_content'   => '',
			'post_status'    => 'inherit',
		);
	...
	public static function verify_ajax_call() {
		...
		if ( ! current_user_can( 'import' ) ) {
			wp_die(
				sprintf(
					__( '%sYour user role isn\'t high enough. You don\'t have permission to import demo data.%s', 'neuron-builder' ),
	...
	public static function process_uploaded_files( $uploaded_files, $log_file_path ) {
		...
		if ( $content_file_info && ! isset( $content_file_info['error'] ) ) {
			...
			$log_added = self::append_to_file(
				sprintf(
					__( 'Content file was not uploaded. Error: %s', 'neuron-builder' ),
					$widget_file_info['error']
				),
				$log_file_path,
				esc_html__( 'Upload files' , 'neuron-builder' )
			);
		}
		...
		if ( $widget_file_info && ! isset( $widget_file_info['error'] ) ) {
			...
			$log_added = self::append_to_file(
				sprintf(
					__( 'Widget file was not uploaded. Error: %s', 'neuron-builder' ),
					$widget_file_info['error']
				),
				$log_file_path,
				esc_html__( 'Upload files' , 'neuron-builder' )
		...
		if ( $customizer_file_info && ! isset( $customizer_file_info['error'] ) ) {
			...
			$log_added = self::append_to_file(
				sprintf(
					__( 'Customizer file was not uploaded. Error: %s', 'neuron-builder' ),
		...
		$log_added = self::append_to_file(
			__( 'The import files were successfully uploaded!', 'neuron-builder' ) . self::import_file_info( $selected_import_files ),
			$log_file_path,
			esc_html__( 'Upload files' , 'neuron-builder' )
		);
	...
	public static function import_file_info( $selected_import_files ) {
		...
		return PHP_EOL .
		sprintf(
			__( 'Initial max execution time = %s', 'neuron-builder' ),
			ini_get( 'max_execution_time' )
		) . PHP_EOL .
		sprintf(
			__( 'Files info:%1$sSite URL = %2$s%1$sData file = %3$s%1$sCustomizer file = %4$s%1$s', 'neuron-builder' ),
			PHP_EOL,
			get_site_url(),
			empty( $selected_import_files['content'] ) ? esc_html__( 'not defined!', 'neuron-builder' ) : $selected_import_files['content'],
			empty( $selected_import_files['customizer'] ) ? esc_html__( 'not defined!', 'neuron-builder' ) : $selected_import_files['customizer']
		);
	}
	 ...
	 */
	public static function get_demo_import_item_categories( $item, $capitalize = false ) {
		$sanitized_categories = array();

		if ( isset( $item['categories'] ) ) {
			foreach ( $item['categories'] as $category ) {
				if ( $capitalize ) {
					$sanitized_categories[] = ucfirst( $category );
				} else {
		...
		if ( ! empty( $sanitized_categories ) ) {
			// $sanitized_categories = implode(' ', $sanitized_categories);
			
			return $sanitized_categories[0];
		}
