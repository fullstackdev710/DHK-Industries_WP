<?php
namespace Neuron\Modules\AssetsManager\Classes;
...
abstract class Assets_Base {
	...
	public function print_metabox( $fields ) {
		?>
		<div class="neuron-admin neuron-admin--metabox-wrapper">
			<div class="neuron-admin__metabox">
				<?php
	...
	public function get_file_field( $field, $saved ) {
		...
		$html .= $this->get_input_field(
			[
				...
				'data-upload_text' => __( 'Upload', 'neuron-builder' ),
				'data-remove_text' => __( 'Delete', 'neuron-builder' ),
	...
	public function get_dropzone_field( $field ) {
		...
		$field['label'] = '<h4><span class="box__dragndrop">' . __( 'Drag & Drop to Upload', 'neuron-builder' ) . '</span></h4>';
		...
		<div class="elementor-dropzone-field">
			<div class="box__input">
				...
				<div class="elementor-button elementor--dropzone--upload__browse">
					<span><?php esc_html_e( 'Click here to browse', 'neuron-builder' ); ?></span>
				</div>
			</div>
			<div class="box__uploading"><?php esc_html_e( 'Uploading&hellip;', 'neuron-builder' ); ?></div>
			<div class="box__success"><?php esc_html_e( 'Done!', 'neuron-builder' ); ?></div>
			<div class="box__error"><?php esc_html_e( 'Error!', 'neuron-builder' ); ?> <span></span>.</div>
		</div>
		<?php
		return ob_get_clean();
	}
	...
	public function get_repeater_field( $field, $saved ) {
		...
		$add_label = isset( $field['add_label'] ) ? $field['add_label'] : __( 'Add item', 'neuron-builder' );
		$row_label = isset( $field['row_label'] ) ? $field['row_label'] : __( 'Row', 'neuron-builder' );
		$row_label_html_args = [
			'id' => 'row_label_' . $js_id,
	...
	private function get_repeater_tools( $field ) {
		$confirm = isset( $field['confirm'] ) ? $field['confirm'] : __( 'Are you sure?', 'neuron-builder' );
		$remove_title = isset( $field['remove_title'] ) ? $field['remove_title'] : __( 'Delete', 'neuron-builder' );
		$toggle_title = isset( $field['toggle_title'] ) ? $field['toggle_title'] : __( 'Edit', 'neuron-builder' );
		$close_title = isset( $field['close_title'] ) ? $field['close_title'] : __( 'Close', 'neuron-builder' );
