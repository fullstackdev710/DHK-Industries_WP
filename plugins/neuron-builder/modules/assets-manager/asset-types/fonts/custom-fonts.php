...
use Neuron\Modules\AssetsManager\AssetTypes\Fonts_Manager;
use Neuron\Plugin;
...
class Custom_Fonts extends Classes\Font_Base {
	...
	public function get_name() {
		return __( 'Custom Fonts', 'neuron-builder' );
	}
	...
	public function add_meta_box() {
		add_meta_box(
			'neuron-font-' . $this->get_type() . 'metabox',
			__( 'Manage Your Font Files', 'neuron-builder' ),
			[ $this, 'render_metabox' ],
			Fonts_Manager::CPT,
			'normal',
			'default'
	...
	public function render_metabox( $post ) {
		...
		$fields = [
			...
			[
				...
				'field_type' => 'html',
				'label' => false,
				'raw_html' => sprintf( '<div class="inline-preview">%s</div>', __( 'The quick brown fox jumps over the lazy dog', 'neuron-builder' ) ),
			],
			[
				'id' => 'toolbar',
				'field_type' => 'toolbar',
		...
		foreach ( $this->get_file_types() as $type => $mine ) {
			$fields[] = [
				...
				'label' => sprintf( __( '%s File', 'neuron-builder' ), strtoupper( $type ) ),
				/* translators: %s: Font file format. */
				'box_title' => sprintf( __( 'Upload font .%s file', 'neuron-builder' ), $type ),
				/* translators: %s: Font file format. */
				'box_action' => sprintf( __( 'Select .%s file', 'neuron-builder' ), $type ),
				'preview_anchor' => 'none',
		...
		$repeater = [
			...
			'id' => 'font_face',
			'label' => false,
			'add_label' => __( 'Add Font Variation', 'neuron-builder' ),
			'toggle_title' => __( 'Edit', 'neuron-builder' ),
			'remove_title' => __( 'Delete', 'neuron-builder' ),
			'field_type' => 'repeater',
			'row_label' => [
				'default' => 'Settings',
	...
	public function wp_handle_upload_prefilter( $file ) {
		...
		if ( $svg_handler::svg_sanitizer_can_run() && ! $svg_handler->sanitize_svg( $file['tmp_name'] ) ) {
			$file['error'] = __( 'Invalid SVG Format, file not uploaded for security reasons', 'neuron-builder' );
		}
	...
	public function get_font_face_from_data( $font_family, $data ) {
		...
		$font_face .= "\tfont-display: " . apply_filters( 'neuron/custom_fonts/font_display', 'swap', $font_family, $data ) . ';' . PHP_EOL;
	...
	public function handle_panel_request( array $data ) {
		...
		if ( empty( $font_face ) ) {
			...
			$error_message = sprintf( __( 'Font %s was not found.', 'neuron-builder' ), $font_family );
	...
	private function get_font_style_options() {
		return [
			...
			'italic' => __( 'Italic', 'neuron-builder' ),
			'oblique' => __( 'Oblique', 'neuron-builder' ),
		];
	}
	...
	private function get_file_type_description( $file_type ) {
		$descriptions = [
			'eot' => __( 'Embedded OpenType, Used by IE6-IE9 Browsers', 'neuron-builder' ),
			'woff2' => __( 'The Web Open Font Format 2, Used by Super Modern Browsers', 'neuron-builder' ),
			'woff' => __( 'The Web Open Font Format, Used by Modern Browsers', 'neuron-builder' ),
			'ttf' => __( 'TrueType Fonts, Used for better supporting Safari, Android, iOS', 'neuron-builder' ),
			'svg' => __( 'SVG fonts allow SVG to be used as glyphs when displaying text, Used by Legacy iOS', 'neuron-builder' ),
		];
