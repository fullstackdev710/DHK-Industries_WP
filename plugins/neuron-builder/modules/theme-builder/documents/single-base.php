...
abstract class Single_Base extends Archive_Single_Base {
	...
	public static function get_title() {
		return __( 'Single', 'neuron-builder' );
	}

	public static function get_editor_panel_config() {
		$config = parent::get_editor_panel_config();

		$config['widgets_settings']['neuron-post-content'] = [
			'show_in_panel' => true,
		];
	...
	public function print_content() {
		...
		if ( $requested_post_id !== $this->post->ID ) {
			...
			if ( $requested_document && ! $requested_document instanceof Section && $requested_document->get_location() !== $this->get_location() ) {
				echo '<div class="elementor-theme-builder-content-area">' . __( 'Content Area', 'neuron-builder' ) . '</div>';
	...
	public static function get_preview_as_options() {
		...
		return [
			'single' => [
				'label' => __( 'Single', 'neuron-builder' ),
	...
	public function get_depended_widget() {
		return Plugin::elementor()->widgets_manager->get_widget_types( 'neuron-post-content' );
	}
	...
	public function preview_error_handler() {
		...
		wp_localize_script( 'neuron-frontend', 'elementorPreviewErrorArgs', [
			/* translators: %s: is the widget name. */
			'headerMessage' => sprintf( __( 'The %s Widget was not found in your template.', 'neuron-builder' ), $depended_widget_title ),
			/* translators: %1$s: is the widget name. %2$s: is the template name.  */
			'message' => sprintf( __( 'You must include the %1$s Widget in your template (%2$s), in order for Elementor to work on this page.', 'neuron-builder' ), $depended_widget_title, '<strong>' . static::get_title() . '</strong>' ),
			'strings' => [
				'confirm' => __( 'Edit Template', 'neuron-builder' ),
			],
