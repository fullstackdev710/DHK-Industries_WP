<?php
namespace Neuron\Modules\ThemeBuilder\Classes;
...
class Locations_Manager {
		...
		if ( $post_id ) {
			...
			if ( $document ) {
				...
				if ( $location_settings && ! $location_settings['edit_in_content'] ) {
					$content = '<div class="elementor-theme-builder-content-area container" style="margin-top: 60px; margin-bottom: 60px;"><h3>' . __( 'Example Content', 'neuron-builder' ) . '</h3><p>This is an example of content, which improvises the page/post look when editing the template.</p></div>';
				}
			}
		}
	...
	public function register_core_location( $location, $args = [] ) {
		if ( ! isset( $this->core_locations[ $location ] ) ) {
			...
			wp_die( esc_html( sprintf( __( 'Location \'%s\' is not a core location.', 'neuron-builder' ), $location ) ) );
		}
