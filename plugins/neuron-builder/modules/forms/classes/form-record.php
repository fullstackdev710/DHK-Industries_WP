...
class Form_Record {
	...
	public function get_formatted_data( $with_meta = false ) {
		$formatted = [];
		$no_label = __( 'No Label', 'neuron-builder' );
		$fields = $this->fields;
	...
	public function validate( $ajax_handler ) {
		foreach ( $this->fields as $id => $field ) {
			 ...
			 * @param Ajax_Handler $ajax_handler An instance of the ajax handler.
			 */
			do_action( "neuron/forms/validation/{$field_type}", $field, $this, $ajax_handler );
		}
		 ...
		 * @param Ajax_Handler $ajax_handler An instance of the ajax handler.
		 */
		do_action( 'neuron/forms/validation', $this, $ajax_handler );
	...
	public function process_fields( $ajax_handler ) {
		foreach ( $this->fields as $id => $field ) {
			 ...
			 * @param Ajax_Handler $ajax_handler An instance of the ajax handler.
			 */
			do_action( "neuron/forms/process/{$field_type}", $field, $this, $ajax_handler );
		}
		 ...
		 * @param Ajax_Handler $ajax_handler An instance of the ajax handler.
		 */
		do_action( 'neuron/forms/process', $this, $ajax_handler );
	}
	...
	private function set_meta() {
		...
		foreach ( $form_metadata as $metadata_type ) {
			switch ( $metadata_type ) {
				...
				case 'page_url':
					$this->meta['page_url'] = [
						'title' => __( 'Page URL', 'neuron-builder' ),
				...
				case 'user_agent':
					$this->meta['user_agent'] = [
						'title' => __( 'User Agent', 'neuron-builder' ),
				...
				case 'remote_ip':
					$this->meta['remote_ip'] = [
						'title' => __( 'Remote IP', 'neuron-builder' ),
	...
	private function sanitize_field( $field, $value ) {
		...
		switch ( $field_type ) {
			...
			default:
				 ...
				 */
				$value = apply_filters( "neuron/forms/sanitize/{$field_type}", $value, $field );
		}
