<?php
namespace Neuron\Modules\Forms\Classes;

use Elementor\Settings;
use Elementor\Widget_Base;
use Neuron\Classes\Utils;
use Neuron\Plugin;
...
class Recaptcha_Handler {
	...
	const OPTION_NAME_SITE_KEY = 'neuron_recaptcha_site_key';

	const OPTION_NAME_SECRET_KEY = 'neuron_recaptcha_secret_key';

	const OPTION_NAME_RECAPTCHA_THRESHOLD = 'neuron_recaptcha_threshold';
	...
	public static function get_setup_message() {
		return __( 'To use reCAPTCHA, you need to add the API Key and complete the setup process in Dashboard > Neuron > Settings > reCAPTCHA.', 'neuron-builder' );
	}
	...
	public function register_scripts() {
		...
		wp_register_script( $script_name, $src, [], NEURON_BUILDER_VERSION, true );
	}
	...
	public function validation( $record, $ajax_handler ) {
		...
		if ( empty( $_POST['g-recaptcha-response'] ) ) {
			$ajax_handler->add_error( $field['id'], __( 'The Captcha field cannot be blank. Please enter a value.', 'neuron-builder' ) );

			return;
		}

		$recaptcha_errors = [
			'missing-input-secret' => __( 'The secret parameter is missing.', 'neuron-builder' ),
			'invalid-input-secret' => __( 'The secret parameter is invalid or malformed.', 'neuron-builder' ),
			'missing-input-response' => __( 'The response parameter is missing.', 'neuron-builder' ),
			'invalid-input-response' => __( 'The response parameter is invalid or malformed.', 'neuron-builder' ),
		];
		...
		if ( 200 !== (int) $response_code ) {
			...
			$ajax_handler->add_error( $field['id'], sprintf( __( 'Can not connect to the reCAPTCHA server (%d).', 'neuron-builder' ), $response_code ) );
		...
		if ( ! $this->validate_result( $result, $field ) ) {
			$message = __( 'Invalid Form - reCAPTCHA validation failed', 'neuron-builder' );
	...
	public function add_field_type( $field_types ) {
		$field_types['recaptcha'] = __( 'reCAPTCHA', 'neuron-builder' );
	...
	public function __construct() {
		...
		add_filter( 'neuron/forms/field_types', [ $this, 'add_field_type' ] );
		add_action( 'neuron/forms/render_field/' . static::get_recaptcha_name(), [ $this, 'render_field' ], 10, 3 );
		add_filter( 'neuron/forms/render/item', [ $this, 'filter_field_item' ] );
		add_filter( 'neuron/editor/localize_settings', [ $this, 'localize_settings' ] );

		if ( static::is_enabled() ) {
			add_action( 'neuron/forms/validation', [ $this, 'validation' ], 10, 2 );
