...
abstract class Form_Base extends Base_Widget {
	...
	public function on_export( $element ) {
		/** @var \Neuron\Modules\Forms\Classes\Action_Base[] $actions */
	...
	protected function make_radio_checkbox_field( $item, $item_index, $type ) {
		...
		$html = '';
		if ( $options ) {
			$html .= '<div class="m-neuron-form__subgroup ' . esc_attr( $item['css_classes'] ) . ' ' . $item['inline_list'] . '">';
			foreach ( $options as $key => $option ) {
				...
				$html .= '<span class="m-neuron-form__option"><input ' . $this->get_render_attribute_string( $element_id ) . '> <label for="' . $html_id . '">' . $option_label . '</label></span>';
			}
			$html .= '</div>';
		}
	...
	protected function form_fields_render_attributes( $i, $instance, $item ) {
		$this->add_render_attribute(
			[
				'field-group' . $i => [
					'class' => [
						'm-neuron-form__field-group--' . $item['field_type'],
				...
				'label' . $i => [
					...
					'class' => 'm-neuron-form__label',
				],
			]
		);


		$this->add_render_attribute( 'field-group' . $i, 'class', 'elementor-repeater-item-' . $item['_id'] . '' );
		...
		if ( ! empty( $item['required'] ) ) {
			$class = 'm-neuron-form__field-group--required';
			if ( ! empty( $instance['mark_required'] ) ) {
				$class .= ' m-neuron-form__field-group--mark-required';
			}
