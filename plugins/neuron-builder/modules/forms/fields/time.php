...
class Time extends Field_Base {
	...
	public function render( $item, $item_index, $form ) {
		$form->add_render_attribute( 'input' . $item_index, 'class', 'm-neuron-form__field neuron-time-field' );
	...
	public function validation( $field, Classes\Form_Record $record, Classes\Ajax_Handler $ajax_handler ) {
		...
		if ( preg_match( '/^(([0-1][0-9])|(2[0-3])):[0-5][0-9]$/', $field['value'] ) !== 1 ) {
			$ajax_handler->add_error( $field['id'], __( 'Invalid Time, Time should be in HH:MM format!', 'neuron-builder' ) );
		}
	}
}
