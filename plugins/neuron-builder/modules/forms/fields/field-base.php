...
abstract class Field_Base {
	...
	public function __construct() {
		...
		add_action( "neuron/forms/render_field/{$field_type}", [ $this, 'field_render' ], 10, 3 );
		add_action( "neuron/forms/validation/{$field_type}", [ $this, 'validation' ], 10, 3 );
		add_action( "neuron/forms/process/{$field_type}", [ $this, 'process_field' ], 10, 3 );
		add_filter( 'neuron/forms/field_types', [ $this, 'add_field_type' ] );
		add_filter( "neuron/forms/sanitize/{$field_type}", [ $this, 'sanitize_field' ], 10, 2 );
		add_action( 'elementor/preview/enqueue_scripts', [ $this, 'add_preview_depends' ] );

		if ( method_exists( $this, 'update_controls' ) ) {
			add_action( 'elementor/element/neuron-form/section_form_fields/before_section_end', [ $this, 'update_controls' ] );
		}
	}
}
