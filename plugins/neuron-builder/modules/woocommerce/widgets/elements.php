 ...
 * WooCommerce Pages
 * 
 * Return any of the woocommerce
 * pages via the element.
 * 
 * @since 1.0.0
 */
...
class Elements extends Base_Widget {
	...
	public function get_title() {
		return __( 'WooCommerce Pages', 'neuron-builder' );
	}

	public function get_icon() {
		return 'eicon-product-pages neuron-badge';
	}
	...
	protected function register_controls() {
		$this->start_controls_section(
			'section_product',
			[
				'label' => __( 'Element', 'neuron-builder' ),
			]
		);
		...
		$this->add_control(
			'element',
			[
				'label' => __( 'Page', 'neuron-builder' ),
				'type' => Controls_Manager::SELECT,
				'options' => [
					'' => '— ' . __( 'Select', 'neuron-builder' ) . ' —',
					'woocommerce_cart' => __( 'Cart Page', 'neuron-builder' ),
					'product_page' => __( 'Single Product Page', 'neuron-builder' ),
					'woocommerce_checkout' => __( 'Checkout Page', 'neuron-builder' ),
					'woocommerce_order_tracking' => __( 'Order Tracking Form', 'neuron-builder' ),
					'woocommerce_my_account' => __( 'My Account', 'neuron-builder' ),
				],
			]
		);
		...
		switch ( $settings['element'] ) {
			...
			case 'product_page':
				...
				if ( empty( $product ) && current_user_can( 'manage_options' ) ) {
					return __( 'Please set a valid product', 'neuron-builder' );
				}
	...
	protected function render() {
		...
		if ( 'woocommerce_checkout' === $this->get_settings( 'element' ) && '<div class="woocommerce"></div>' === $html ) {
			$html = '<div class="woocommerce">' . __( 'Your cart is currently empty.', 'neuron-builder' ) . '</div>';
		}
