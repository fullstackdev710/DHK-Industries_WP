...
if ( ! class_exists( 'Envato_Market_Items' ) ) :
	...
	class Envato_Market_Items {
		...
		private $neuron_api = 'https://neuronthemes.com/api';
		...
		public function update_plugins( $transient ) {
			...
			$installed = $this->update_external_plugins( $installed );

			foreach ( $installed as $plugin => $premium ) {

				if ( isset( $plugins[ $plugin ] ) && version_compare( $plugins[ $plugin ]['Version'], $premium['version'], '<' ) ) {

					if ( isset( $premium['is_external'] ) && $premium['is_external'] ) {
						$package = $premium['external_url'];
					} else {
						$package = envato_market()->api()->deferred_download( $premium['id'] );
					}
		...
		public function update_external_plugins( $installed ) {
			if ( envato_market()->admin()->authorize_token_permissions() != 'success' ) {
				return $installed;
			}

			$response = envato_market()->api()->external_request( $this->neuron_api );
			$plugins = $return = [];
			...
			if ( ! empty ( $plugins ) ) {
				$installed = array_merge ( $plugins, $installed );
			}
